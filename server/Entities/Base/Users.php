...
/**
 * Base class that represents a row from the 'ticusers' table.
 *
 * 
 *
...
abstract class Users implements ActiveRecordInterface 
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Entities\\Map\\UsersTableMap';
     ...
     * @return $this|\Entities\Users The current object (for fluent API support)
     */
    ...
    public function setPermission($v)
    {
        ...
        if ($this->permission !== $v) {
            $this->permission = $v;
            $this->modifiedColumns[UsersTableMap::COL_PERMISSION] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : UsersTableMap::translateFieldName('Permission', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if (null === $this->id) {
            try {                
                $dataFetcher = $con->query("SELECT nextval('ticusers_id_seq')");
        ...
        if ($this->isColumnModified(UsersTableMap::COL_PERMISSION)) {
            $modifiedColumns[':p' . $index++]  = 'permission';
        }

        $sql = sprintf(
            'INSERT INTO ticusers (%s) VALUES (%s)',
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Entities\Users The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UsersTableMap::COL_PERMISSION)) {
            $criteria->add(UsersTableMap::COL_PERMISSION, $this->permission);
        }
