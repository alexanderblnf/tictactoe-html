...
/**
 * Base class that represents a query for the 'ticusers' table.
 *
 * 
 *
 ...
 * @method     ChildUsersQuery orderByPermission($order = Criteria::ASC) Order by the permission column
 *
 ...
 * @method     ChildUsersQuery groupByPermission() Group by the permission column
 *
 ...
 * @method     ChildUsers findOneByPermission(int $permission) Return the first ChildUsers filtered by the permission column *
 ...
 * @method     ChildUsers requireOneByPermission(int $permission) Return the first ChildUsers filtered by the permission column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildUsers[]|ObjectCollection findByPermission(int $permission) Return ChildUsers objects filtered by the permission column
...
abstract class UsersQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, email, password, firstname, lastname, permission FROM ticusers WHERE id = :p0';
        try {
    ...
    public function filterByPermission($permission = null, $comparison = null)
    {
        if (is_array($permission)) {
            ...
            if (isset($permission['min'])) {
                $this->addUsingAlias(UsersTableMap::COL_PERMISSION, $permission['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($permission['max'])) {
                $this->addUsingAlias(UsersTableMap::COL_PERMISSION, $permission['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UsersTableMap::COL_PERMISSION, $permission, $comparison);
    }
